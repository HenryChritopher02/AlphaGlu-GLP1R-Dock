...
from utils.paths import (
    APP_VERSION, BASE_GITHUB_URL_FOR_DATA, APP_ROOT, VINA_PATH_LOCAL,
    RECEPTOR_DIR_LOCAL, CONFIG_DIR_LOCAL,
    LIGAND_PREP_DIR_LOCAL, LIGAND_UPLOAD_TEMP_DIR, ZIP_EXTRACT_DIR_LOCAL,
    DOCKING_OUTPUT_DIR_LOCAL,SCRUB_PY_LOCAL_PATH, MK_PREPARE_LIGAND_PY_LOCAL_PATH
)
...
DIABETES_TARGETS = {
    "Alpha-Glucosidase (2QMJ)": {
        "pdbqt": "alphaglucosidase.pdbqt",
        "config": "alphaglucosidase.txt"
    },
    "GLP1-R allosteric site (9IVM)": {
        "pdbqt": "glp1allo.pdbqt",
        "config": "glp1allo.txt"
    }
}
...
def display_diabetes_docking_procedure():
    ...
    with tab1:
        ...
        if input_method == "Upload PDBQT/ZIP":
            ...
            st.markdown("Using **Quercetin** as example.")
            example_smi = "O=c1c(c(O)cc(O)c2)c2oc(c3ccc(O)c(O)c3)c1O"
            st.code(example_smi)
            if st.button("Process Example"):
                with st.spinner("Processing Quercetin..."):
                    result = convert_smiles_to_pdbqt(
                        example_smi, "quercetin_example", LIGAND_PREP_DIR_LOCAL, 
                        7.4, False, False, SCRUB_PY_LOCAL_PATH, MK_PREPARE_LIGAND_PY_LOCAL_PATH
                    )
                    if result:
                        new_ligands.append(result['pdbqt_path'])
                        st.success("Quercetin added to docking list.")
                    else:
